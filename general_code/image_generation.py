...
from general_code.utils import load_image, image_sampling, save_image


def squeeze_img(
    pil_img: Image.Image, orientation: int, ratio: Union[float, Tuple[float]]
) -> Image.Image:
    """Squeeze the image.
...
def load_batch_images(
    count: int, begin_id: int, img_path: str, verbose: bool = True
) -> List[Image.Image]:
    if verbose:
...
def generate_batch_images(
    count: int, id_begin: int, fonts: List[str], language: str
    ...
    text_color = [random_text_color, standard_text_color][
        np.random.choice([0, 1], p=np.array([0.2, 0.8]).ravel())
    ]

    orientation = np.random.choice([0, 1], p=np.array([0.1, 0.9]).ravel())
    space_width = random.uniform(0, 1.5)  # space between words
    character_spacing = int(
        np.clip(np.random.normal(30, 20), 0, 70)
    ...
    stroke_fill = [random_stroke_fill, standard_stroke_fill][
        np.random.choice([0, 1], p=np.array([0.4, 0.6]).ravel())
    ]
    ...
    generator = GeneratorFromDict(count=count, fonts=[font], **prop_dict)
...
def generate_images(
    ...
    for i in tqdm(range(((total_num - 1) // gen_batch_size) + 1)):
        ...
        batch_img_ls, batch_label_df = generate_batch_images(
            batch_count, batch_id_begin, fonts, language
        )
        label_df = (
            batch_label_df
            if label_df is None
            else pd.concat([label_df, batch_label_df])
        )
        if need_save:
            save_images(
                batch_img_ls,
                [
                    f"{id:08d}"
                    for id in range(batch_id_begin, batch_id_begin + len(batch_img_ls))
                ],
                gen_image_path,
                False,
            )
...
def saved_images_sampling(
    ...
    for i in tqdm(range(batch_num)):
        ...
        batch_img_ls = load_batch_images(
            count=batch_count, begin_id=batch_id_begin, img_path=img_path, verbose=False
        )
        batch_sample_imgs = [
            image_sampling(img, sample_num, width, height) for img in batch_img_ls
        ]
        ...
        if need_save:
            save_images(
                ...
                [
                    f"{id:08d}_{s_id:04d}"
                    for id in range(
                        batch_id_begin, batch_id_begin + len(batch_sample_imgs)
                    )
